...
import { useInvoiceDetails, markInvoicePrinted } from '@/features/staff/api';
...
export default function InvoicePrintPage() {
  ...
  const { data: invoice, isLoading, error, refetch } = useInvoiceDetails(invoiceId);
  ...
  const recordFirstPrintAndPrint = async () => {
    try {
      await markInvoicePrinted(invoiceId, 'In hóa đơn lần đầu');
      await refetch();
      setTimeout(() => {
        window.print();
      }, 500);
  ...
  const handleConfirmReprint = async () => {
    ...
    try {
      await markInvoicePrinted(invoiceId, printReason.trim());
      await refetch();
  ...
  if (error || !invoice) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50 text-red-600 text-sm p-4 text-center">
        Lỗi tải thông tin hóa đơn hoặc không tìm thấy hóa đơn.
      </div>
    );
  }
